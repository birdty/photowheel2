...
-(void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    {
        if ( [SendEmailController canSendEmail] ) [names addObject:@"emailPhotos"];
        [names addObject:@"confirmDeletePhotoAlbmu"];
    }
    else
    {
...
-(void)confirmDeletePhotoAlbum
{
    ...
    if ( [name length] > 0 )
    {
        message = [NSString stringWithFormat:@"Delete the photo album \%s\". This actions cannot be undone.", name];
    }
    else
    {
...
-(void)layoutForLandScape
{
    ...
    UIImage * image = [UIImage imageNamed:@"stack-viewer-bg-landscape-right.png"];
