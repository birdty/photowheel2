...
@property (nonatomic, assign) NSInteger percentScrolledIntoFirstVislblePage;

@property (nonatomic, strong) MainSlideShowViewController * slideShowViewController;
...
-(void)viewDidLoad
{
    ...
    // set to display contents behind status bar
...
-(void)showActionMenu:(id)sender
{
    ...
    if ( [SendEmailController canSendEmail] )
    {
...
-(void)printCurrentPhoto
{
    ...
    UIPrintInteractionCompletionHandler completionHandler = ^(UIPrintInteractionController * printController, BOOL completed, NSError * error)
    {
        ...
        if ( completed && error )
        {
            NSLog(@"FAILED! due to error is domain %@ with error code: %u", error.domain, error.code);
        }
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    {
        [self setSlideShowViewController: (MainSlideShowViewController *)[segue destinationViewController]];
        
        [[self slideShowViewController] setDelegate:[self delegate]];
        [[self slideShowViewController] setStartIndex:[self currentIndex]];
    }
}
...
#pragma mark - Email and SendEmailControlletDelegate methods
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    if ( [self slideShowViewController] != nil )
    {
        [self setCurrentIndex:[[self slideShowViewController] currentIndex]];
        [self scrollToIndex:[[self slideShowViewController] currentIndex]];
        [self setSlideShowViewController:nil];
    }
    else
    {
        [self setCurrentIndex:[self startAtIndex]];
...
-(void)setTitleWithCurrentIndex
{
    ...
    NSString * title = [NSString stringWithFormat:@"%1$i of %2$i", index, count, nil];
...
-(void)willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
    ...
    if ( offset )
    {
        ...
        [self setPercentScrolledIntoFirstVislblePage:([self firstVisiblePageIndexBeforeRotation] * pageWidth) / pageWidth];
        
    }
    else
    {
        [self setFirstVisiblePageIndexBeforeRotation:0];
        [self setPercentScrolledIntoFirstVislblePage:offset / pageWidth];
    }
}
...
-(void)layoutScrollViewSubviews
{
    ...
    for(PhotoBrowserPhotoView * view in subviews )
    {
        ...
        [view setMaxMinZoomScaleForCurrentBounds];
    ...
    CGFloat newOffset = ([self firstVisiblePageIndexBeforeRotation] * pageWidth) + ([self percentScrolledIntoFirstVislblePage] * pageWidth);
...
-(UIImage *)sepiaImageFromImage:(UIImage *)myImage
{
    ...
    [sepiaFilter setValue:[NSNumber numberWithFloat:0.9] forKey:@"inputIntensity"];
...
-(UIImage *)enhanceImage:(UIImage *)myImage
{
    CIImage * myCIImage = [CIImage imageWithCGImage:[myImage CGImage]];
    
    NSDictionary * options = @{CIDetectorImageOrientation: [[myCIImage properties] valueForKey:kCGImagePropertyOrientation] };
    
    
    NSArray * autoAdjustmentFilters = [myCIImage autoAdjustmentFiltersWithOptions:options];
    
    CIImage * enchancedCIImage = myCIImage;
    
    for(CIFilter * filter in autoAdjustmentFilters)
    {
        [filter setValue:enchancedCIImage forKey:kCIInputImageKey];
        enchancedCIImage = [filter outputImage];
    }
    
    CGImageRef enchangedCGImage = [[self ciContext] createCGImage:enchancedCIImage fromRect:[enchancedCIImage extent]];
    
    UIImage * enhancedImage = [UIImage imageWithCGImage:enchangedCGImage];
    
    CFRelease(enchangedCGImage);
...
-(IBAction)zoomToFaces:(id)sender
{
    ...
    if ( [faces count] > 0 )
    {
        ...
        [cropFilter setValue:largeCIImage forKey:kCIInputEVKey];
    ...
    {
        UIAlertView * noFacesAlert = [[UIAlertView alloc] initWithTitle:@"No Faces" message:@"Sorry, I couldn't fin any faces in the picture" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
